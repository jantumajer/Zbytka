...
RATIOS<-function(rwl)
{
  ...
  for(i in 1:ncol(rwl))
  {
    RATIO<-mean(rwl[PER_1,i])/mean(rwl[PER_2,i])
    P<-wilcox.test(rwl[PER_1,i],rwl[PER_2,i])$p.val
    if(RATIO>1&&P<0.05)
    {
      RESP[i]<-"NEG"
    }
...
PCGA<-function(DATA)
{
  ...
  SORT_POL_1<-vector(mode="numeric")
  for(i in 1:nrow(POL_COORD))
  {
    SORT_POL_1[i]<-which(POL_COORD[,1]==sort(POL_COORD[,1])[i])
  }
  list(pca=PCA,imp=IMP,rank=SORT_POL_1,pol.coord=POL_COORD,pop=DATA[,colnames(CI.DATA)],period=rownames(CI.DATA))
}

###########################################
plot.PCGA<-function(PCGA.STATS,col.vec=NULL, ident=NULL,scale=FALSE)
{
  if(scale==T)
  {
    PCGA.STATS$pca$rot[,1:2]<-PCGA.STATS$pca$rot[,1:2]/max(abs(PCGA.STATS$pca$rot[,1:2]))
  }		
  plot(0,0,type="n",xlim=range(-c(PCGA.STATS$pca$rot[,1],PCGA.STATS$pca$rot[,2]),c(PCGA.STATS$pca$rot[,1],PCGA.STATS$pca$rot[,2])),
       ylim=range(-c(PCGA.STATS$pca$rot[,1],PCGA.STATS$pca$rot[,2]),c(PCGA.STATS$pca$rot[,1],PCGA.STATS$pca$rot[,2])),
	 main="",xlab=round(PCGA.STATS$imp[2,1],2),ylab=round(PCGA.STATS$imp[2,2],2),
       # main="",xlab="",ylab="",
  ...
  if(length(col.vec)==0)
  {
    for(i in 1:nrow(PCGA.STATS$pca$rot))
    {
      arrows(0,0,PCGA.STATS$pca$rot[PCGA.STATS$rank[i],1],PCGA.STATS$pca$rot[PCGA.STATS$rank[i],2],col=hsv(i/(nrow(PCGA.STATS$pca$rot)*1.2)),lwd=2,length=0.1)
    }
  }
  if(length(col.vec)>0)
  {
    for(i in 1:nrow(PCGA.STATS$pca$rot))
    {
      arrows(0,0,PCGA.STATS$pca$rot[PCGA.STATS$rank[i],1],PCGA.STATS$pca$rot[PCGA.STATS$rank[i],2],col=col.vec[PCGA.STATS$rank[i]], lwd=2,length=0.1)
    }
  }
  if(length(ident>0))
  {
    ...
    {
      arrows(0,0,PCGA.STATS$pca$rot[PCGA.STATS$rank[ident[i]],1],PCGA.STATS$pca$rot[PCGA.STATS$rank[ident[i]],2],col="black",lwd=2,length=0.1)
    }
  }
}
...
#################################################################
### Dopocet klouzaveho Rbar pro problemove responder chronologie
#################################################################

output <- data.frame(yr=c(1965:2002), Ho_NEG=NA, Ze_NEG=NA)

Ho <- Houkvice_vla[timespan.row.vla, c("h_04vl", "h_05vl", "h_18vl")]
Ze <- Zernov_vla[timespan.row.vla, c("zer_06", "zer_17", "zer_28")]

for (i in (1:38)){
	output[i,2] <- mean(cor(Ho[c(i:(i+20)),])[upper.tri(cor(Ho))])
	output[i,3] <- mean(cor(Ze[c(i:(i+20)),])[upper.tri(cor(Ze))])


	}

###write.table(output, "F:/zbytka/cluster_oprava/chron_kor/pomocneRbar.txt", sep="\t", col.names=TRUE, row.names=TRUE, quote=TRUE, na="NA")
